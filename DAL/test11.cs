...
namespace DAL
{
	...
	public partial class test11
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "test11"); 
		}
		...
		public Model.test11 GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,name from test11 ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			Model.test11 model=new Model.test11();
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.test11 DataRowToModel(DataRow row)
		{
			Model.test11 model=new Model.test11();
			if (row != null)
			{
