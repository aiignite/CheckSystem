...
{
    public sealed class Nsl23924 : ControllerBase
    {
        ...
        public Nsl23924(string name) : base(name)
        {
        ...
        public void SingleErgodicProgramTpsFromDataFile(string index)
        {
            ...
            try
            {
                ...
                for (var i = 1; i < lines.Count; i++)
                {
                    ...
                    {
                        if (lastRedAddr == 0xFF && bRegAddr != 0x87)
                        {
                            lastRedAddr = bRegAddr;
                            dataList.Add(new Dictionary<byte, byte>());
                            dataList[dataList.Count - 1].Add(bRegAddr, bRegData);

                            if (bRegAddr < 0x87)
                                toCrcData.Add(bRegData);
                        }
                        else if (lastRedAddr != 0xFF && bRegAddr != 0x87)
                        {
                            if (lastRedAddr + 0x01 == bRegAddr)
                            {
                                lastRedAddr = bRegAddr;
                                dataList[dataList.Count - 1].Add(bRegAddr, bRegData);
            ...
            var totalCrc = CALC_CRC(_tps929120TiCrcInfo, toCrcData.ToArray());
            dataList.Add(new Dictionary<byte, byte>());
            dataList[dataList.Count - 1].Add(0x87, (byte)totalCrc);
                        ...
                        if (TpsSendCmd(GetSendData((byte)(BroadcastWrite1BytesDevAddrByte + p), 0x97, new byte[] { 0x01 })))
                        {
                            if (true)
                            {
                                ...
                                foreach (var d in dataList)
                                {
                                    ...
                                    //}
                                    else if (d.Count >= 1 && d.Count < 24) // 按最大1字节分组
                                    {
                                        var baseIndex = -1;
