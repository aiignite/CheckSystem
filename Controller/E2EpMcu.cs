...
{
    [Description("CAN-Product,E2-EPMCU")]
    public sealed class E2EpMcu : ControllerBase
    {
        public CanBus Can;

        public E2EpMcu(string name)
            : base(name)
        {
        ...
        private const uint MaskLevel1 = 0x544D738Au;
        private const uint MaskLevel2 = 0xDA8E29B3u;
            ...
            while (_keepNetworkThread.IsAlive)
            {
                ...
                lock (_lockSend)
                {
                    Can.SendStandardCanData(
                       0x481, new byte[] { 0x81, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 });
                }
            }
        }
        ...
        public string AppFilePath = @"C:\Projs\2022\E2驻车控制器\替代料芯片\EP33L_EPMCU_APP_V5.0.01_20220811_Release.s19";
