...
namespace Controller
{
    ...
    public sealed class WirelessChargingModule : ControllerBase
    {
        ...
        public void LoadTrack(int mtcIndex, int mtcLen)
        {
            ...
            if (!string.IsNullOrEmpty(TrackBarcode))
            {
                ...
                lock (_lockDb)
                {
                    ...
                    var allServerData = _db.Queryable<ECUData>().Where(it => it.Barcode == TrackBarcode && it.TrackInfo == mtc).ToList();

                    {
                        if (!allServerData.Any())
                            allServerData = _db.Queryable<ECUData>().Where(it => it.IsUsage == 0 || it.IsUsage == 2).ToList();
                        else
                        {
                        ...
                        if (allServerData is null || allServerData.Count == 0)
                        {
                            ...
                            {
                                allServerData = _db.Queryable<ECUData>().Where(it => it.IsUsage == 1 && it.Barcode == TrackBarcode && it.TrackInfo == mtc).ToList();
        ...
        private bool CanFdUdsRid272(byte[] routineControlOptBytes, out byte[] echoBytes)
        {
            ...
            var isFirstRecvOk = _bRid272WaitHandle.Wait(500);
            ...
            if (isFirstRecvOk)
            {
                ...
                var isSencondRecvOk = _bRid272WaitHandle.Wait(500);
