...
namespace Controller
{
    ...
    public sealed class WirelessChargingModule : ControllerBase
    {
        ...
        {
            _dbName = string.IsNullOrEmpty(Name) ? HighPrecisionTimer.GetTimestamp().ToString() : Name;
        ...
        //private SqlSugarClient _localDb;
        ...
        public void LoadTrack(int mtcIndex, int mtcLen)
        {
            ...
            if (!string.IsNullOrEmpty(TrackBarcode))
            {
                ...
                lock (_lockDb)
                {
                    ...
                    {
                        var serverData = _db.Queryable<ECUData>().Where(it => it.Barcode == TrackBarcode && it.TrackInfo == mtc).First();
                        if (serverData is null)
                        {
                            serverData = _db.Queryable<ECUData>().Where(it => it.IsUsage == 0).OrderBy(f => f.CreateTime).First();
                            if (serverData is null)
                            {
                        ...
                        {
                            _trackEcuData = null;
                            TrackResult = "NG 根据当前条码获取到已经使用过的 ECU ID";
                            return;
                        }
                        ...
                        var result = _db.Updateable(_trackEcuData).ExecuteCommand();
        ...
        public void SaveTrack()
        {
            ...
            //{
            //    var getData = _localDb.Queryable<ECUData>().Where(it => it.EcuId == _trackEcuData.EcuId).First();
            //    if (getData != null)
            //    {
            //        var dr = _localDb.Deleteable(getData).Where(it => it.EcuId == _trackEcuData.EcuId).ExecuteCommand();
            //    }
            //}
        }

        private void SaveTrackTemp()
        {
            if (_trackEcuData != null)
            ...
            //else
            //{
            //    _localDb.Updateable(tempData).Where(f => f.EcuId == tempData.EcuId).ExecuteCommand();
            //}
        ...
        public bool EnterFactoryMode(byte[] seedBytes)
        {
            try
            {
                ...
                //    // D2C5545194
                //    // D2 C5 54 51 94
                //    var tpSeed = new byte[] { 0xD2, 0xC5 };
                //    // 计算工厂模式密钥
                //    var tpkey = CalcKey((ushort)(tpSeed[0] * 256 + tpSeed[1]));
                //    var tpkeyBytes = BitConverter.GetBytes(tpkey).Reverse().ToArray();
                //    var tpcrcByte = CalculateChecksum(tpSeed, tpkeyBytes);
                //}
        ...
        public void ReadApp()
        {
            AppNo = ValueHelper.GetDecimal(ReadDid(0xF1, 0xC1)).ToString();
            AppVersion = Encoding.ASCII.GetString(ReadDid(0xF1, 0xD1));
        }
        ...
        public void ReadCal()
        {
            CalNo = ValueHelper.GetDecimal(ReadDid(0xF1, 0xC2)).ToString();
            CalVersion = Encoding.ASCII.GetString(ReadDid(0xF1, 0xD2));
        }
        ...
        {
            BaseModulePartNumber = ValueHelper.GetDecimal(ReadDid(0xF1, 0xCC)).ToString();
            BaseModuleAlphaCode = Encoding.ASCII.GetString(ReadDid(0xF1, 0xDC));
        }
        ...
        {
            EndModulePartNumber = ValueHelper.GetDecimal(ReadDid(0xF1, 0xCB)).ToString();
            EndModuleAlphaCode = Encoding.ASCII.GetString(ReadDid(0xF1, 0xDB));
        }

        [Description("R,VPPS")]
        public string VPPS = string.Empty;
        [Description("读VPPS")]
        public void ReadVPPS() => VPPS = Encoding.ASCII.GetString(ReadDid(0xF0, 0xAB));

        [Description("R,DUNS")]
        public string DUNS = string.Empty;
        [Description("读DUNS")]
        public void ReadDUNS() => DUNS = Encoding.ASCII.GetString(ReadDid(0xF0, 0xB3));
        ...
        public void WriteAndReadDDI(string ddiHex)
        {
            ...
            {
                DDI = ValueHelper.GetHextStr(ReadDid(didHi, didLo)).Replace(" ", "");
                return;
            }
        ...
        public void WriteAndReadMtc()
        {
            if (_trackEcuData is null)
            {
                var tp = Encoding.ASCII.GetString(ReadDid(0xF0, 0xB4));
            ...
            Thread.Sleep(15);
            var tpr = Encoding.ASCII.GetString(ReadDid(0xF0, 0xB4));
        ...
        public void WriteAndReadECUID()
        {
            if (_trackEcuData is null)
            {
                var tp = ValueHelper.GetHextStr(ReadDid(0xF0, 0xF3)).Replace(" ", "");
            ...
            Thread.Sleep(15);
            var tpr = ValueHelper.GetHextStr(ReadDid(0xF0, 0xF3)).Replace(" ", "");
        ...
        public void DebugReadEUCID()
        {
            ...
            //Thread.Sleep(150);
            ECUID = string.Empty;
            ECUID = ValueHelper.GetHextStr(ReadDid(0xF0, 0xF3)).Replace(" ", "");
        }
        ...
        private byte[] ReadDid(byte didHi, byte didLo)
        {
            ...
            //byte[] echo;
            //if (CanFD.CanBusWithUds.TryReadData(DiagnosticReqCanId, DiagnosticRecvCanId, CanBus.CanType.Extended, CanBus.CanProtocol.CanFd, didHi, didLo, out echo, pendingByte: 0x00))
            //{
            //    return echo;
            //}
        ...
        private bool CanFdUds22(byte didHi, byte didLo, out byte[] echo)
        {
            ...
            var taskFirstWait = Task.Factory.StartNew(() =>
            {
                return _bReadDidWaitHandle.Wait(1500);
            });
            var taskFirstSend = Task.Factory.StartNew(() =>
            {
                CanFD.SendExtendedCanFdData(DiagnosticReqCanId, firstSend);
            ...
            if (isFirstRecvOk)
            {
                if (_uds22Buffer.Count == 8)
                {
                    var b1 = _uds22Buffer[0];
                    var b2 = _uds22Buffer[1];
                    var b3 = _uds22Buffer[2];
                    var b4 = _uds22Buffer[3];

                    if (b1.GetByteHighOrder() == 0x00 && b2 == 0x62 && b3 == didHi && b4 == didLo)
                    {
                        var datalen = b1.GetByteLowOrder();
                        if (datalen >= 3 && datalen <= 7)
                        {
                            echo = new byte[datalen - 3];
                            Array.Copy(_uds22Buffer.ToArray(), 4, echo, 0, datalen - 3);
                            return true;
                        }
                    }
                }
                else if (_uds22Buffer.Count > 8)
                {
                    var b1 = _uds22Buffer[0];
                    var b2 = _uds22Buffer[1];
                    var b3 = _uds22Buffer[2];
                    ...
                    {
                        var datalen = b2;
                        if (datalen >= 3 && datalen <= 62 && _uds22Buffer.Count >= datalen + 2)
                        {
                            echo = new byte[datalen - 3];
                            Array.Copy(_uds22Buffer.ToArray(), 5, echo, 0, datalen - 3);
                            return true;
                        }
                    }
                }
            ...
            out byte wrtieHistory, out byte[] m4, out byte[] m5)
        {
            ...
            //if (SecurityAccess0D0EResult == "OK")
            {
        ...
        private bool CanFdUdsRid272(byte[] routineControlOptBytes, out byte[] echoBytes)
        {
            ...
            {
                return _bRid272WaitHandle.Wait(1000);
            });
            var tSend = Task.Factory.StartNew(() =>
            {
                CanFD.SendExtendedCanFdData(DiagnosticReqCanId, firstSendBytes);
            });
            Task.WaitAll(tWait, tSend);
            ...
            if (tWait.Result)
            {
                ...
                _bRid272 = true;
                tWait = Task.Factory.StartNew(() =>
                {
                    return _bRid272WaitHandle.Wait(1000);
                });
                tSend = Task.Factory.StartNew(() =>
                {
                    CanFD.SendExtendedCanFdData(DiagnosticReqCanId, secondSendBytes);
                });
                Task.WaitAll(tWait, tSend);
                _bRid272 = false;
                _bRid272WaitHandle.Reset();
                _bInJnjectKey = false;
                if (tWait.Result && _udsRid272Buffer.Count == 64)
                {
