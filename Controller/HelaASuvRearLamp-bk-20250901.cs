...
namespace Controller
{
    ...
    public sealed class HelaASuvRearLamp : ControllerBase
    {
        ...
        public void WriteFazitIdAndSerialNumberByBarcode()
        {
            ...
            {
                FazitId = "NG 条码格式不对";
                SerialNumber = "NG 条码格式不对";
                return;
            }

            var day = ToWriteBarcode.Substring(17, 3);
            var serialNumber = ToWriteBarcode.Substring(10, 6);

            var year = DateTime.Now.Year;
            var dateTime = DateTime.Parse(string.Format("{0}/01/01", year)).AddDays(int.Parse(day) - 1);
            ...
            var fazit = string.Format("9FH-HFL{0}.{1}.{2}{3}{4}", dateTime.Day.ToString().PadLeft(2, '0'),
                dateTime.Month.ToString().PadLeft(2, '0'), dateTime.Year.ToString().Substring(2, 2), deviceId.ToString().PadLeft(4, '0'), int.Parse(serialNumber).ToString().PadLeft(4, '0'));
            var serialNo = string.Format("{0}.{1}.{2}{3}{4}", dateTime.Day.ToString().PadLeft(2, '0'),
                dateTime.Month.ToString().PadLeft(2, '0'), dateTime.Year.ToString().Substring(2, 2),
                deviceId.ToString().PadLeft(4, '0'), int.Parse(serialNumber).ToString().PadLeft(4, '0'))
                .PadLeft(20, '0');
            //Console.WriteLine(fazit.Length);
            //Console.WriteLine(serialNo.Length);
            ...
            if (serialNumberRead == serialNo)
                ...
                SerialNumber = "NG " + serialNumberRead;
        }
