...
namespace Controller
{
    ...
    */
    ...
    public sealed class MdfHeadlampAnalysis : ControllerBase
    {
        ...
        public MdfHeadlampAnalysis(string name) : base(name)
        {
        ...
        public void AnalyzeImage(string srcName, string mdfSrcName)
        {
            ClearAnalysisResult();
            ...
            if (!Images.ContainsKey(mdfSrcName) || Images[mdfSrcName] == null || Images[mdfSrcName].Empty())
            {
                AnalysisResult = "FAILED MDF图像不存在";
                return;
            }
                                                        ...
                                                        if (inflectionPoint != default)
                                                        {
                                                            ...
                                                            using (var systemMat = imageSrc.Clone())
                                                            {
                                                                ...
                                                                using (var dstSystemImage = systemMat.Clone(roiRect))
                                                                {
                                                                    ...
                                                                    MdfAnalysis(Images[mdfSrcName], DegreeToPixelPoint(inflectionPoint, 0, 0), inflectionPoint, dicGray["EMax"]);
                                                                }
        ...
        public double MdfHDegree = -9999;

        private void MdfAnalysis(Mat mdfSrc, Point pH0V0, Point inflectionPoint, double eMaxGray)
        {

            //debug mdf 光型
            {
                var mdfMat = mdfSrc.Clone();

                var kernel = Cv2.GetStructuringElement(MorphShapes.Rect, new Size(21, 3));
                var clahe = Cv2.CreateCLAHE(clipLimit: 2.0, tileGridSize: new Size(8, 8));

                var tpMdfStandard1 = DegreeToPixelPoint(inflectionPoint, -3.45, 0);
                var tpMdfStandard2 = DegreeToPixelPoint(inflectionPoint, -2.55, 0);

                // 如果图像是彩色图像，将其转换为灰度图像来进行处理
                if (mdfMat.Channels() == 3)
                    Cv2.CvtColor(mdfMat, mdfMat, ColorConversionCodes.BGR2GRAY);
                AppendImage("mdf灰度图像", mdfMat);

                var toCheckMdfGray = mdfMat.Clone();

                // 自适应直方图均衡化（CLAHE）
                clahe.Apply(mdfMat, mdfMat);
                AppendImage("mdf自适应直方图均衡化（CLAHE）", mdfMat);

                // 高斯滤波降噪
                Cv2.GaussianBlur(mdfMat, mdfMat, new Size(5, 5), sigmaX: 1.5);
                AppendImage("mdf高斯滤波降噪", mdfMat);

                Cv2.Threshold(mdfMat, mdfMat, 0, 255, ThresholdTypes.Otsu | ThresholdTypes.Binary);
                AppendImage("mdf图像OTSU二值化图像", mdfMat);

                Rect mdfOuteRect1;
                if (MyCamera.TryGetMaxContourOuterRect(mdfMat, out mdfOuteRect1))
                {
                    var mdfRoi = mdfSrc.Clone(mdfOuteRect1);
                    AppendImage("mdf图像roi", mdfRoi);

                    // 如果图像是彩色图像，将其转换为灰度图像来进行处理
                    if (mdfRoi.Channels() == 3)
                        Cv2.CvtColor(mdfRoi, mdfRoi, ColorConversionCodes.BGR2GRAY);
                    AppendImage("mdf图像roi灰度图像", mdfRoi);

                    var listGradPoints = new List<Point>();
                    var listRect = new List<Rect>();

                    var toCheckV = new List<float> { 0.2f, 1.1f, 2.0f };

                    {
                        foreach (var v in toCheckV)
                        {
                            var mdfCheckLinePixel = DegreeToPixelPoint(inflectionPoint, 0, v);
                            var dxStart = PixelPointToDegree(inflectionPoint, new Point(mdfOuteRect1.X, mdfCheckLinePixel.Y));
                            var dxEnd = PixelPointToDegree(inflectionPoint, new Point(mdfOuteRect1.X + mdfOuteRect1.Width, mdfCheckLinePixel.Y));
                            var maxG = 0d;
                            Point maxGp1 = default;
                            Point maxGp2 = default;
                            for (var x = -4d; x <= dxEnd.X; x += 0.05f)
                            {
                                var p1 = DegreeToPixelPoint(inflectionPoint, x, v);
                                var r1 = GetGrayRoiByDegree(inflectionPoint, x, v, 0.1);
                                //r1 = new Rect(new Point(p1.X - 1, p1.Y - 1), new Size(3, 3));
                                var lx1 = GetGray(new Mat(toCheckMdfGray, r1));

                                var p2 = DegreeToPixelPoint(inflectionPoint, x + 0.1, v);
                                var r2 = GetGrayRoiByDegree(inflectionPoint, x + 0.1, v, 0.1);
                                //r2 = new Rect(new Point(p2.X - 1, p2.Y - 1), new Size(3, 3));
                                var lx2 = GetGray(new Mat(toCheckMdfGray, r2));

                                listRect.Add(r1);
                                listRect.Add(r2);

                                Console.WriteLine($"x:{x}, lx1:{lx1}, lx2:{lx2}");

                                var g = Math.Round(Math.Log10(lx2) - Math.Log10(lx1), 2, MidpointRounding.AwayFromZero);
                            ...
                            //if (maxGp1 != default && maxGp2 != default && maxG > 0.01)
                            //    listGradPoints.Add(new Point((maxGp1.X + maxGp2.X) / 2, (maxGp1.Y + maxGp2.Y) / 2));

                            if (maxGp1 != default && maxGp2 != default && maxG > 0.01)
                                listGradPoints.Add(new Point(maxGp2.X, maxGp2.Y));
                        }

                        if (toCheckMdfGray.Channels() == 1)
                            Cv2.CvtColor(toCheckMdfGray, toCheckMdfGray, ColorConversionCodes.GRAY2BGR);

                        foreach (var p in listRect)
                            Cv2.Rectangle(toCheckMdfGray, p, Scalar.Purple);

                        var listGrad = new List<double>();

                        foreach (var p in listGradPoints)
                        {
                            var hStr = PixelPointToDegree(inflectionPoint, p).X;
                            listGrad.Add(hStr);
                            Cv2.PutText(toCheckMdfGray, hStr.ToString(CultureInfo.InvariantCulture), new Point(p.X + 2, p.Y + 0), HersheyFonts.HersheyComplex, 0.5, Scalar.Blue);
                            Cv2.Circle(toCheckMdfGray, p, 1, Scalar.Red, -1);
                        }

                        if (listGrad.Any())
                        {
                            MdfHDegree = listGrad.Max(Math.Abs);
                        }

                        Cv2.Circle(toCheckMdfGray, inflectionPoint, 1, Scalar.Red, -1);
                        Cv2.Rectangle(toCheckMdfGray, mdfOuteRect1, Scalar.Purple);

                        Cv2.Line(toCheckMdfGray, new Point(tpMdfStandard1.X, 0), new Point(tpMdfStandard1.X, toCheckMdfGray.Height), Scalar.Purple);
                        Cv2.Line(toCheckMdfGray, new Point(tpMdfStandard2.X, 0), new Point(tpMdfStandard2.X, toCheckMdfGray.Height), Scalar.Purple);

                        AppendImage("mdf图像梯度检测结果", toCheckMdfGray);
                    }

                    //// 自适应直方图均衡化（CLAHE）
                    //clahe.Apply(mdfRoi, mdfRoi);
                    //AppendImage("mdf图像roi自适应直方图均衡化（CLAHE）", mdfRoi);

                    //// 高斯滤波降噪
                    //Cv2.GaussianBlur(mdfRoi, mdfRoi, new Size(5, 5), sigmaX: 1.5);
                    //AppendImage("mdf图像roi高斯滤波降噪", mdfRoi);

                    //Cv2.Threshold(mdfRoi, mdfRoi, 0, 255, ThresholdTypes.Otsu | ThresholdTypes.Binary);
                    //AppendImage("mdf图像图像roi OTSU二值化图像", mdfRoi);

                    //Cv2.MorphologyEx(mdfRoi, mdfRoi, MorphTypes.Close, kernel);
                    //AppendImage("mdf图像图像roi 形态学闭操作（连接断裂边缘）", mdfRoi);


                    //Cv2.Threshold(mdfRoi, mdfRoi, eMaxGray * 0.1,255 , ThresholdTypes.Binary);
                    //AppendImage("mdf图像图像10%EMax二值化图像", mdfRoi);

                    //var mdfCheckP1 = DegreeToPixelPoint(inflectionPoint, 0, 0.2);
                    //var mdfCheckP2 = DegreeToPixelPoint(inflectionPoint, 0, 1.1);
                    //var mdfCheckP3 = DegreeToPixelPoint(inflectionPoint, 0, 2.0);

                    //var mdfShowCheckLine = mdfRoi.Clone();
                    //if (mdfShowCheckLine.Channels() == 1)
                    //    Cv2.CvtColor(mdfShowCheckLine, mdfShowCheckLine, ColorConversionCodes.GRAY2BGR);
                    //Cv2.Line(mdfShowCheckLine, new Point(0, mdfCheckP1.Y - mdfOuteRect1.Y), new Point(mdfShowCheckLine.Width, mdfCheckP1.Y - mdfOuteRect1.Y), Scalar.Red);
                    //Cv2.Line(mdfShowCheckLine, new Point(0, mdfCheckP2.Y - mdfOuteRect1.Y), new Point(mdfShowCheckLine.Width, mdfCheckP2.Y - mdfOuteRect1.Y), Scalar.Green);
                    //Cv2.Line(mdfShowCheckLine, new Point(0, mdfCheckP3.Y - mdfOuteRect1.Y), new Point(mdfShowCheckLine.Width, mdfCheckP3.Y - mdfOuteRect1.Y), Scalar.Blue);
                    //AppendImage("mdf图像截止线检测H位置", mdfShowCheckLine);

                    //Point mdfP1 = default;
                    //Point mdfP2 = default;
                    //Point mdfP3 = default;

                    //for (var x = 0; x < mdfRoi.Width; x++)
                    //{
                    //    var gray1 = 0;
                    //    var gray2 = 0;
                    //    var gray3 = 0;
                    //    if (mdfCheckP1.Y - mdfOuteRect1.Y > 0)
                    //        gray1 = mdfRoi.At<byte>(mdfCheckP1.Y - mdfOuteRect1.Y, x);
                    //    if (mdfCheckP2.Y - mdfOuteRect1.Y > 0)
                    //        gray2 = mdfRoi.At<byte>(mdfCheckP2.Y - mdfOuteRect1.Y, x);
                    //    if (mdfCheckP3.Y - mdfOuteRect1.Y > 0)
                    //        gray3 = mdfRoi.At<byte>(mdfCheckP3.Y - mdfOuteRect1.Y, x);
                    //    //var gray2 = mdfRoi.At<byte>(mdfCheckP2.Y - mdfOuteRect1.Y, x);
                    //    //var gray3 = mdfRoi.At<byte>(mdfCheckP3.Y - mdfOuteRect1.Y, x);

                    //    if (gray1 == 255 && mdfP1 == default)
                    //        mdfP1 = new Point(x + mdfOuteRect1.X, mdfCheckP1.Y);
                    //    if (gray2 == 255 && mdfP2 == default)
                    //        mdfP2 = new Point(x + mdfOuteRect1.X, mdfCheckP2.Y);
                    //    if (gray3 == 255 && mdfP3 == default)
                    //        mdfP3 = new Point(x + mdfOuteRect1.X, mdfCheckP3.Y);

                    //    if (mdfP1 != default && mdfP2 != default && mdfP3 != default)
                    //        break;
                    //}
                    ...
                    //var mdfDst = mdfSrc.Clone();
                    //if (mdfDst.Channels() == 1)
                    //    Cv2.CvtColor(mdfDst, mdfDst, ColorConversionCodes.GRAY2BGR);

                    //if (mdfP1 != default)
                    //{
                    //    mdfH1 = PixelPointToDegree(inflectionPoint, mdfP1).X;
                    //    Cv2.Circle(mdfDst, mdfP1, 1, Scalar.Red, -1);
                    //    Cv2.PutText(mdfDst, mdfH1.ToString(CultureInfo.InvariantCulture), new Point(mdfP1.X + 2, mdfP1.Y + 0), HersheyFonts.HersheyComplex, 0.5, Scalar.Red);
                    //}

                    //if (mdfP2 != default)
                    //{
                    //    mdfH2 = PixelPointToDegree(inflectionPoint, mdfP2).X;
                    //    Cv2.Circle(mdfDst, mdfP2, 1, Scalar.Green, -1);
                    //    Cv2.PutText(mdfDst, mdfH2.ToString(CultureInfo.InvariantCulture), new Point(mdfP2.X + 2, mdfP2.Y + 0), HersheyFonts.HersheyComplex, 0.5, Scalar.Green);
                    //}

                    //if (mdfP3 != default)
                    //{
                    //    mdfH3 = PixelPointToDegree(inflectionPoint, mdfP3).X;
                    //    Cv2.Circle(mdfDst, mdfP3, 1, Scalar.Blue, -1);
                    //    Cv2.PutText(mdfDst, mdfH3.ToString(CultureInfo.InvariantCulture), new Point(mdfP3.X + 2, mdfP3.Y + 0), HersheyFonts.HersheyComplex, 0.5, Scalar.Blue);
                    //}


                    //Cv2.Line(mdfDst, new Point(tpMdfStandard1.X, 0), new Point(tpMdfStandard1.X, mdfDst.Height), Scalar.Purple);
                    //Cv2.Line(mdfDst, new Point(tpMdfStandard2.X, 0), new Point(tpMdfStandard2.X, mdfDst.Height), Scalar.Purple);

                    //Cv2.Line(mdfDst, new Point(0, pH0V0.Y), new Point(mdfDst.Width, pH0V0.Y), Scalar.Yellow);
                    //Cv2.Line(mdfDst, new Point(pH0V0.X, 0), new Point(pH0V0.X, mdfDst.Height), Scalar.Yellow);
                    //Cv2.Circle(mdfDst, inflectionPoint, 1, Scalar.Red, -1);
                    //Cv2.PutText(mdfDst, "inflection", new Point(inflectionPoint.X + 2, inflectionPoint.Y + 0), HersheyFonts.HersheyComplex, 0.5, Scalar.Red);
                    //Cv2.Circle(mdfDst, pH0V0, 1, Scalar.Red, -1);
                    //Cv2.PutText(mdfDst, "HV", new Point(pH0V0.X + 2, pH0V0.Y + 0), HersheyFonts.HersheyComplex, 0.5, Scalar.Red);
                    //AppendImage("mdf图像截止线V检测位置", mdfDst);

                    //var tpMdf7HV = PixelPointToDegree(inflectionPoint, new Point(1310, 1936));
                    //var tpNgXH = DegreeToPixelPoint(inflectionPoint, -3.64, 0).X;
                }

                //var mdfDst = Cv2.ImRead(mdfPath);
                //if (mdfDst.Channels() == 1)
                //    Cv2.CvtColor(mdfDst, mdfDst, ColorConversionCodes.GRAY2BGR);

                //var mdfCheckP1 = CalcPixelPointByInflection(inflectionPoint, 0, 0.2);
                //var mdfCheckP2 = CalcPixelPointByInflection(inflectionPoint, 0, 1.1);
                //var mdfCheckP3 = CalcPixelPointByInflection(inflectionPoint, 0, 2.0);

                //Cv2.Line(mdfDst, new Point(0, mdfCheckP1.Y), new Point(mdfDst.Width, mdfCheckP1.Y), Scalar.Red);
                //Cv2.Line(mdfDst, new Point(0, mdfCheckP2.Y), new Point(mdfDst.Width, mdfCheckP2.Y), Scalar.Green);
                //Cv2.Line(mdfDst, new Point(0, mdfCheckP3.Y), new Point(mdfDst.Width, mdfCheckP3.Y), Scalar.Blue);
                //AppendImage("mdf图像截止线检测位置", mdfDst);

                //Point mdfP1 = default;
                //Point mdfP2 = default;
                //Point mdfP3 = default;

                //for (var x = 0; x < mdfDst.Width; x++)
                //{
                //    var gray1 = mdfMat.At<byte>(mdfCheckP1.Y, x);
                //    var gray2 = mdfMat.At<byte>(mdfCheckP2.Y, x);
                //    var gray3 = mdfMat.At<byte>(mdfCheckP3.Y, x);

                //    if (gray1 == 255 && mdfP1 == default)
                //        mdfP1 = new Point(x, mdfCheckP1.Y);
                //    if (gray2 == 255 && mdfP2 == default)
                //        mdfP2 = new Point(x, mdfCheckP2.Y);
                //    if (gray3 == 255 && mdfP3 == default)
                //        mdfP3 = new Point(x, mdfCheckP3.Y);
                ...
                //if (mdfP1 != default)
                //    mdfH1 = CalcDegreePointByInflection(inflectionPoint, mdfP1).X;
                //if (mdfP2 != default)
                //    mdfH2 = CalcDegreePointByInflection(inflectionPoint, mdfP2).X;
                //if (mdfP3 != default)
                //    mdfH3 = CalcDegreePointByInflection(inflectionPoint, mdfP3).X;

                //var tpMdfHV = CalcDegreePointByInflection(inflectionPoint, new Point(252 + maxRect.X, 66 + maxRect.Y));
            }

            //using (var mdfRoi = mdfSrc.Clone(roiRect))
            //{
            //    Mat imageSrcGray;
            //    Mat imageSrcClahe;
            //    Mat imageSrcGaussianBlur;
            ...
            //    AppendImage("MDF原图灰度图像", imageSrcGray);
            //    AppendImage("MDF原图自适应直方图均衡化（CLAHE）", imageSrcClahe);
            //    AppendImage("MDF原图高斯滤波降噪", imageSrcGaussianBlur);
            //    AppendImage("MDF原图OTSU二值化图像", imageSrcOtsu);
            //    AppendImage("MDF原图形态学闭操作（连接断裂边缘）", imageSrcClose);
            //    AppendImage("MDF原图边缘检测", imageSrcCannyEdge);
        ...
        private Rect GetGrayRoiByDegree(Point inflectionPoint, double degreeH, double degreeV,
            double offsetDegreeH = 0.02d, double offsetDegreeV = 0.02d, bool isInflectionPointHv = false)
        {
