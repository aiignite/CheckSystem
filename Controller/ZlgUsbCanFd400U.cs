...
namespace Controller
{
    ...
    public sealed class ZlgUsbCanFd400U : ControllerBase
    {
        public CanBus ZlgCanChannel0;
        public CanBus ZlgCanChannel1;
        public CanBus ZlgCanChannel2;
        public CanBus ZlgCanChannel3;

        private readonly IntPtr _deviceHandle;
        private const int Null = 0;
        private readonly IntPtr _channelHandle0;
        private readonly IntPtr _channelHandle1;
        private readonly IntPtr _channelHandle2;
        private readonly IntPtr _channelHandle3;
        private Property _property;
        private readonly Recvdatathread _can0RecvDataThread;
        private readonly Recvdatathread _can1RecvDataThread;
        private readonly Recvdatathread _can2RecvDataThread;
        private readonly Recvdatathread _can3RecvDataThread;
        ...
        public ZlgUsbCanFd400U(string name)
            : base(name)
        {
            var deviceInfo = new DeviceInfo(Define.ZcanUsbcanfd400U, 4);
            ...
            for (var i = 0; i < 4; i++)
            {
                if (usbCanfd)
                {
                    if (!SetCanfdStandard(0, i)) //设置CANFD标准
                        return;
                }
                else
                {
                    if (!canfdDevice && !SetBaudrate(1000000, i))
                    {
                        return;
                    }
                }
            ...
            _channelHandle2 = Method.ZCAN_InitCAN(_deviceHandle, 2, pConfig);
            _channelHandle3 = Method.ZCAN_InitCAN(_deviceHandle, 3, pConfig);
            ...
            if (Null != (int)_channelHandle2)
            {
                if (usbCanfd)
                    if (!SetResistanceEnable(2))
                        return;

                if (canfdDevice && !SetFilter(2))
                    return;

                if (Method.ZCAN_StartCAN(_channelHandle2) != Define.StatusOk)
                {

                }
                else
                {
                    _can2RecvDataThread = new Recvdatathread();
                    _can2RecvDataThread.SetChannelHandle(_channelHandle2);
                    _can2RecvDataThread.SetStart(true);
                    _can2RecvDataThread.RecvCanData += AddCan2Data;
                    _can2RecvDataThread.RecvFdData += AddCan2Data;

                    ZlgCanChannel2 = new ZlgUsbCanChannel(this, name + "_can2", 2);
                }
            }

            if (Null != (int)_channelHandle3)
            {
                if (usbCanfd)
                    if (!SetResistanceEnable(3))
                        return;

                if (canfdDevice && !SetFilter(3))
                    return;

                if (Method.ZCAN_StartCAN(_channelHandle3) != Define.StatusOk)
                {

                }
                else
                {
                    _can3RecvDataThread = new Recvdatathread();
                    _can3RecvDataThread.SetChannelHandle(_channelHandle3);
                    _can3RecvDataThread.SetStart(true);
                    _can3RecvDataThread.RecvCanData += AddCan3Data;
                    _can3RecvDataThread.RecvFdData += AddCan3Data;

                    ZlgCanChannel3 = new ZlgUsbCanChannel(this, name + "_can3", 3);
                }
            }
        }
        ...
        private void AddCan2Data(ZcanReceiveData[] data, uint len)
        {
            if (ZlgCanChannel2 == null)
                return;
            ...
            ZlgCanChannel2.ReceiveCanDatas(canPackages.ToArray());
        }

        private void AddCan2Data(ZcanReceiveFdData[] data, uint len)
        {
            if (ZlgCanChannel2 == null)
                return;
        ...
        private void AddCan3Data(ZcanReceiveData[] data, uint len)
        {
            if (ZlgCanChannel3 == null)
                return;
            ...
            ZlgCanChannel3.ReceiveCanDatas(canPackages.ToArray());
        }

        private void AddCan3Data(ZcanReceiveFdData[] data, uint len)
        {
            if (ZlgCanChannel3 == null)
                return;
            ...
            foreach (var tsd in toSendData)
            {
                ...
                if (tsd.Any(f => f.CanProtocol == CanBus.CanProtocol.Can))
                {
                    ...
                    if (zcanTransmitDatas.Any())
                    {
                        ...
                        else if (canChIndex == 2)
                        {
                            var result = Method.ZCAN_Transmit(_channelHandle2, ptr, (uint)len);
                            Marshal.FreeHGlobal(ptr);
                        ...
                        else if (canChIndex == 3)
                        {
                            var result = Method.ZCAN_Transmit(_channelHandle3, ptr, (uint)len);
                            Marshal.FreeHGlobal(ptr);
                ...
                else
                {
                    ...
                    if (zcanTransmitDatas.Any())
                    {
                        ...
                        {
                            var result = Method.ZCAN_TransmitFD(_channelHandle2, ptr, (uint)len);
                            Marshal.FreeHGlobal(ptr);
                        ...
                        {
                            var result = Method.ZCAN_TransmitFD(_channelHandle3, ptr, (uint)len);
                            Marshal.FreeHGlobal(ptr);
