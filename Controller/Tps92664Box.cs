...
{
    public sealed class Tps92664Box : ControllerBase
    {
        public MySerialPort MySerialPort;

        public Tps92664Box(string name)
            : base(name)
        {
        ...
        public void SendCmdPhase()
        {
            lock (_lockDevList)
            {
                foreach (var dev in _lstMatrix)
                {
                    ...
                    pDatas.AddRange(sendPhaseH);
                    pDatas.AddRange(sendPhaseL);
                    MySerialPort.SendCommand(SendWrtieData(0xBF, 0x15, pDatas), 5);
                }
            }
        }

        public void SendCmdReadWidth()
        {
            foreach (var dev in _lstMatrix)
            {
                var sendBytes = SendReadData(dev.DevId, 0x29, 20);
                //var sendStr = ValueHelper.GetHextStr(sendBytes).Replace(" ", "");
                MySerialPort.SendCommand(sendBytes, 5);
            }
        }
            ...
            byte devId, byte startAddr, int bytesLen)
        {
            ...
            switch (bytesLen)
            {
                ...
                case 20:
                    sendBytes.Add((byte)Commands.ReadCmdOf20Byte);
                    sendBytes.Add(devId);
                    sendBytes.Add(startAddr);
                    sendBytes.AddRange(ValueHelper.IbmCrc16(sendBytes));
                    break;
