...
{
    [Description("LIN-Product,GlobalE2LBHVSM座椅加热模块")]
    public sealed class GlobalE2LBHvsmSeatHeatingModule : ControllerBase
    {
        ...
        public GlobalE2LBHvsmSeatHeatingModule(string name) : base(name)
        {
        ...
        ~GlobalE2LBHvsmSeatHeatingModule()
        {
            Dispose();
        }
        ...
        public void ReadError()
        {
            ...
            if (LinWithBaudRate10417 != null)
            {
                ...
                {
                    byte[] echo;
                    if (LinWithBaudRate10417.SendSlaveLin(0x12, out echo, timeOutMs: 200))
                    {
                        if (echo.Length >= 4)
                        {
                            var newbs = new byte[4];
                            Array.Copy(echo, 0, newbs, 0, 4);
        ...
        {
            AppVer = string.Empty;
            var result = ReadDid(9, 0xFC, 0x06, _nad);
        ...
        public void ReadBootVer()
        {
            ...
            var result = ReadDid(9, 0xFC, 0x03, _nad);
        ...
        {
            CalVer = string.Empty;
            var result = ReadDid(_cRow == Row.Front ? 31 : 30, 0xF1, 0xC2, _nad);
        ...
        public void ReadPartNo()
        {
            ...
            lock (_lockSend)
            {
                ...
                if (LinWithBaudRate10417.SendMasterLinAndRecvSingleSlaveLin(0x3C, 0x3D, b1, out echo1, delayMs: 50) && echo1 != null && echo1.Length == 8)
                {
                    if (ValueHelper.GetHextStr(echo1).Replace(" ", "").ToUpper().StartsWith(ValueHelper.GetHextStr(_nad).ToUpper() + "05F2"))
                    {
                        if (LinWithBaudRate10417.SendMasterLinAndRecvSingleSlaveLin(0x3C, 0x3D, b2, out echo2, delayMs: 50) && echo2 != null && echo2.Length == 8)
                        {
                            if (ValueHelper.GetHextStr(echo1).Replace(" ", "").ToUpper().StartsWith(ValueHelper.GetHextStr(_nad).ToUpper() + "05F2"))
                            {
                                result.Add(echo1[3]);
                ...
                if (LinWithBaudRate10417.SendMasterLinAndRecvSingleSlaveLin(0x3C, 0x3D, b1, out echo1, delayMs: 50) && echo1 != null && echo1.Length == 8)
                {
                    if (ValueHelper.GetHextStr(echo1).Replace(" ", "").ToUpper().StartsWith(ValueHelper.GetHextStr(_nad).ToUpper() + "05F2"))
                    {
                        if (LinWithBaudRate10417.SendMasterLinAndRecvSingleSlaveLin(0x3C, 0x3D, b2, out echo2, delayMs: 50) && echo2 != null && echo2.Length == 8)
                        {
                            ...
                            {
                                if (LinWithBaudRate10417.SendMasterLinAndRecvSingleSlaveLin(0x3C, 0x3D, b3, out echo3, delayMs: 50) && echo3 != null && echo3.Length == 8)
                                {
                                    if (ValueHelper.GetHextStr(echo3).Replace(" ", "").ToUpper().StartsWith(ValueHelper.GetHextStr(_nad).ToUpper() + "05F2"))
                                    {
                                        if (LinWithBaudRate10417.SendMasterLinAndRecvSingleSlaveLin(0x3C, 0x3D, b4, out echo4, delayMs: 50) && echo4 != null && echo4.Length == 8)
                                        {
        ...
        #region 反读mtc并生成序列号和二维码

        [Description("反读取生产序列号并生成二维码")]
        public void ReadMtcAndGenerateBarcode(string generalPartNo, string generalVpps, string seeyaoDuns, string track1, string track2, string track3)
        {
            Barcode = string.Empty;
            ModbusBarcode = string.Empty;
                                    ...
                                    if (ValueHelper.GetHextStr(echo3).Replace(" ", "").ToUpper().StartsWith(ValueHelper.GetHextStr(_nad).ToUpper() + "05F2"))
                                    {
                                        if (LinWithBaudRate10417.SendMasterLinAndRecvSingleSlaveLin(0x3C, 0x3D, b4, out echo4, delayMs: 50) && echo4 != null && echo4.Length == 8)
                                        {
                                            if (ValueHelper.GetHextStr(echo4).Replace(" ", "").ToUpper().StartsWith(ValueHelper.GetHextStr(_nad).ToUpper() + "05F2"))
                                            {
                                                ...
                                                if (result.Any() && result.Count == 10)
                                                {
                                                    var resultHexStr = ValueHelper.GetHextStr(result.ToArray()).Replace(" ", "");
                                                    if (!resultHexStr.Contains("00") && !resultHexStr.Contains("FF"))
                                                    {
                                                        var readStr = ValueHelper.GetStringByAsciiBytes(result.ToArray(), false);
                                                        ...
                                                        {
                                                            {
                                                                ...
                                                                var mtcYear =  readStr.Substring(0, 2);
                                                                var mtcMonth = readStr.Substring(2, 2);
                                                                var mtcDay = int.Parse(readStr.Substring(4, 2));
                                                                var mtcSerialNo = int.Parse(readStr.Substring(6, 4));

                                                                var outDate = string.Format("20{0}/{1}/{2} 00:00:00", mtcYear, mtcMonth, mtcDay);

                                                                var tracInfo = string.Format("1A{0}{1}{2}{3}{4}{5}", DateTime.Parse(outDate).Year.ToString().Substring(2, 2),
                                                                    DateTime.Parse(outDate).DayOfYear.ToString().PadLeft(3, '0'), track1, track2, track3,
                                                                    mtcSerialNo.ToString().PadLeft(4, '0'));
