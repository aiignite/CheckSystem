...
{
    public sealed class MatrixTps92662With3Q1 : ControllerBase
    {
        ...
        [Description("R/W,是否打印log")]
        public bool IsPrintTrace = true;

        [Description("R/W,是否打开PhaseShift")]
        public bool IsPSON = false;
        ...
        public MatrixTps92662With3Q1(string name)
            : base(name)
        {
            #region HZ数据字典
        ...
        {
            if (devAddr.Length != 3 && devAddr.Length != 4 && devAddr.Length != 5)
                return;
            ...
            if (upToDeviceType == 0)
            {
                ...
                var deviceAddr = new DeviceIdUpto8Tps92662DevicesOnOneBus(
                    devAddr[2].ToString(),
                    devAddr[1].ToString(),
                    devAddr[0].ToString(), isBroadcast, 0);
            ...
            else if (upToDeviceType == 1)
            {
                ...
                var deviceAddr = new DeviceIdUpto8Tps92662DevicesOnOneBus(
                    ...
                    devAddr[1].ToString(),
                    devAddr[0].ToString(),
                    isBroadcast, 1, devAddr[2].ToString(), devAddr[3].ToString());
            ...
            else
            {
                ...
                var deviceAddr = new DeviceIdUpto8Tps92662DevicesOnOneBus(
                    ...
                    devAddr[1].ToString(),
                    devAddr[0].ToString(),
                    isBroadcast, 2, devAddr[2].ToString(), devAddr[3].ToString(), devAddr[4].ToString());
        ...
        private void MySerialPort_PushSciMsg(string name, byte[] datas)
        {
            ...
            if (_isReadingAllRegValue)
            {
                _readAllRegValueBuffer.Add(ValueHelper.GetHextStr(datas).Replace(" ", ""));
            }

            if (_bReadReg0XB0_0XB1_0XB2)
            {
                if (str.StartsWith("D2") && str.Length == 20 && str.Substring(4, 2) == "B0")
                {
                    REG_0xB0 = "0x" + str.Substring(10, 2);
                    REG_0xB1 = "0x" + str.Substring(12, 2);
                    REG_0xB2 = "0x" + str.Substring(14, 2);
                    Console.WriteLine("width={0}, read reg b0&b1&b2: " + str, _nowBroadcastWidth);
                }
            }
        }
        ...
        public void SysConfig()
        {
            ...
            {
                var data = SendWrtieData(_lstMatrix[i].DeviceId.DevId, 0x80, new byte[] { IsPSON ? (byte)0x16 : (byte)0x06 });
                MySerialPort.SendCommand(data);
                Thread.Sleep(5);
        ...
        private ushort _nowBroadcastWidth = 1023;

        [Description("设置广播模式占空比")]
        public void SetBroadcastWidth(ushort widthVal)
        {
            if (widthVal >= 0 && widthVal <= 1023)
            {
                _nowBroadcastWidth = widthVal;
                var keys = _lstMatrix.Keys.ToList();

                for (int i = 0; i < keys.Count; i++)
                {
                    var k = keys[i];
                    if (_lstMatrix[k].DeviceId.DevId == 0xBF)
                    {
                        for (int j = 1; j <= 12; j++)
                        {
                            _lstMatrix[k].LedTurnOn(j, phaseVal: 0, widthVal: widthVal);
                        }
                    }
                }
            }
        ...
        public void ReadIcIdentificationRegister(string devAddr)
        {
            ...
            if (devAddr == "FFF" || (devAddr.Length != 3 && devAddr.Length != 4 && devAddr.Length != 5))
                return;
            ...
            else if (upToDeviceType == 1)
            {
                var deviceAddr = new DeviceIdUpto8Tps92662DevicesOnOneBus(
                    ...
                    devAddr[1].ToString(),
                    devAddr[0].ToString(),
                    false, 1, devAddr[2].ToString(), devAddr[3].ToString());
            ...
            else
            {
                var deviceAddr = new DeviceIdUpto8Tps92662DevicesOnOneBus(
                    ...
                    devAddr[1].ToString(),
                    devAddr[0].ToString(),
                    false, 2, devAddr[2].ToString(), devAddr[3].ToString(), devAddr[4].ToString());
        ...
        private class SingleMatrix
        {
            ...
            public ushort _boardCastWidth = 1023;
        ...
        private List<string> _readAllRegValueBuffer = new List<string>();
        private bool _isReadingAllRegValue;

        [Description("读取所有寄存器并保存")]
        public void ReadAllRegValue(string devAddr)
        {
            _readAllRegValueBuffer.Clear();

            if (devAddr == "FFF" || (devAddr.Length != 3 && devAddr.Length != 4 && devAddr.Length != 5))
                return;

            var listRegAddr = new List<byte>();
            for (var i = 0x00; i <= 0x0F; i++)
                listRegAddr.Add((byte)i);
            ...
            for (var i = 0x30; i <= 0x3F; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0x40; i <= 0x4F; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0x50; i <= 0x5F; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0x60; i <= 0x6F; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0x70; i <= 0x78; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0x80; i <= 0x84; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0x90; i <= 0x97; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0xA0; i <= 0xA1; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0xB0; i <= 0xB2; i++)
                listRegAddr.Add((byte)i);
            for (var i = 0xFF; i <= 0xFF; i++)
                listRegAddr.Add((byte)i);
            ...
            _isReadingAllRegValue = true;

            for (int k = 0; k < listRegAddr.Count; k++)
            {
                var toReadRegAddr = listRegAddr[k];
                ...
                int slaveAddr;
                {
                    if (MySerialPort != null)
                    {
                        if (upToDeviceType == 0)
                        {
                            ...
                            var sendBytes = SendReadData(dev.DevId, toReadRegAddr, 1);
                            var sendStr = ValueHelper.GetHextStr(sendBytes).Replace(" ", "");
                            sendStrList.Add(sendStr);
                            MySerialPort.SendCommand(sendBytes);
                            Thread.Sleep(25);
                        }
                        else if (upToDeviceType == 1)
            ...
            Thread.Sleep(500);
            _isReadingAllRegValue = false;

            var valueList = new List<string>();
            if (_readAllRegValueBuffer.Count == sendStrList.Count)
            {
                for (int i = 0; i < sendStrList.Count; i++)
                {
                    var recvValue = _readAllRegValueBuffer[i];
                    var sendValue = sendStrList[i];
                    if (recvValue.StartsWith(sendValue) && recvValue.Length >= sendValue.Length)
                    {
                        var regValue = string.Format("{0}{1}", recvValue[10], recvValue[11]);
                        valueList.Add(regValue);
                    }
                }

                if (valueList.Count == listRegAddr.Count)
                {
                    Console.WriteLine("Read log:");
                    foreach (var value in _readAllRegValueBuffer)
                    {
                    ...
                    for (int i = 0; i < listRegAddr.Count; i++)
                    {
                        Console.WriteLine("ADDR {0}h = {1}h", ValueHelper.GetHextStr(listRegAddr[i]), valueList[i]);
                    }
                }
            }
        }
        ...
        #region Read reg 0xB0/0XB1

        [Description("R,REG_0xB0")]
        public string REG_0xB0 = string.Empty;
        [Description("R,REG_0xB1")]
        public string REG_0xB1 = string.Empty;
        [Description("R,REG_0xB2")]
        public string REG_0xB2 = string.Empty;

        private bool _bReadReg0XB0_0XB1_0XB2;

        [Description("ReadReg0XB0_0XB1_0XB2")]
        public void ReadReg0XB0_0XB1_0XB2(int addr)
        {
            REG_0xB0 = string.Empty;
            REG_0xB1 = string.Empty;
            REG_0xB2 = string.Empty;

            if (addr >= 0 && addr <= 7)
            {
                var bits = Convert.ToString(addr, 2).PadLeft(3, '0');
                ...
                if (MySerialPort != null)
                {
                    var dev = new DeviceIdUpto8Tps92662DevicesOnOneBus(
                               bits[2].ToString(),
                               bits[1].ToString(),
                               bits[0].ToString(), false, 0);

                    var writeClearBytes = SendWrtieData(dev.DevId, 0xB0, new byte[2]);
                    MySerialPort.SendCommand(writeClearBytes);
                    Thread.Sleep(50);

                    var sendBytes = SendReadData(dev.DevId, 0xB0, 3);
                    var sendStr = ValueHelper.GetHextStr(sendBytes).Replace(" ", "");

                    _bReadReg0XB0_0XB1_0XB2 = true;
                    MySerialPort.SendCommand(sendBytes);
                    Thread.Sleep(250);
                    _bReadReg0XB0_0XB1_0XB2 = false;
                }
            }
        }
