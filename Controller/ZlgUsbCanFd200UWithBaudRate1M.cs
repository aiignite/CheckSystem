...
namespace Controller
{
    ...
    public sealed class ZlgUsbCanFd200UWithBaudRate1M : ControllerBase
    {
        ...
        public ZlgUsbCanFd200UWithBaudRate1M(string name)
            : base(name)
        {
            ...
            if (usbCanfd)
            {
                if (!SetCanfdStandard(0, 1)) //设置CANFD标准
                    return;
                if (!setFdBaudrate(1000000, 1000000, 1)) //设置CANFD标准
                    return;
            }
            else
            {
        ...
        private bool setFdBaudrate(int abaud, int dbaud, int channel_index)
        {
        ...
        {
            private readonly ZlgUsbCanFd200UWithBaudRate1M _zlgUsbCanFd200U;
            private readonly int _canIndex;
            private const int MaxCanDataLen = 60;

            public ZlgUsbCanChannel(
                ZlgUsbCanFd200UWithBaudRate1M zlgUsbCanFd200Ustring, string name, int canIndex)
