...
{
    public sealed class PesControlPlc : ControllerBase
    {
        ...
        public PesControlPlc(string name) : base(name) { }

        ~PesControlPlc() => Dispose();
        ...
        private void Daemon()
        {
            ...
            while (_daemonTh.IsAlive)
            {
                ...
                if (readBs.IsSuccess)
                {
                    ...
                    PreStart = readBs.Content[12] == 0x01;
                }
            }
        }
        ...
        public void ClearBarcodeBuff()
        {
            lock (_lockBarcode)
                _barcodeScanList.Clear();
        }
