...
namespace BLL
{
	...
	{
		private readonly DAL.pdProcessBaseDetail dal=new DAL.pdProcessBaseDetail();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.pdProcessBaseDetail GetModelByCache(int id)
		{
			
			string CacheKey = "pdProcessBaseDetailModel-" + id;
			...
			return (Model.pdProcessBaseDetail)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdProcessBaseDetail> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdProcessBaseDetail> DataTableToList(DataTable dt)
		{
			List<Model.pdProcessBaseDetail> modelList = new List<Model.pdProcessBaseDetail>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.pdProcessBaseDetail model;
