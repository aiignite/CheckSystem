...
namespace BLL
{
	...
	{
		private readonly DAL.systemTreeInfo dal=new DAL.systemTreeInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.systemTreeInfo GetModelByCache(int id)
		{
			
			string CacheKey = "systemTreeInfoModel-" + id;
			...
			return (Model.systemTreeInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.systemTreeInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.systemTreeInfo> DataTableToList(DataTable dt)
		{
			List<Model.systemTreeInfo> modelList = new List<Model.systemTreeInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.systemTreeInfo model;
