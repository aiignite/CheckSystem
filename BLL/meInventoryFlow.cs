...
namespace BLL
{
	...
	{
		private readonly DAL.meInventoryFlow dal=new DAL.meInventoryFlow();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.meInventoryFlow GetModelByCache(string ID)
		{
			
			string CacheKey = "meInventoryFlowModel-" + ID;
			...
			return (Model.meInventoryFlow)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.meInventoryFlow> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.meInventoryFlow> DataTableToList(DataTable dt)
		{
			List<Model.meInventoryFlow> modelList = new List<Model.meInventoryFlow>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.meInventoryFlow model;
