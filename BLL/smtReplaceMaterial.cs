...
namespace BLL
{
	...
	{
		private readonly DAL.smtReplaceMaterial dal=new DAL.smtReplaceMaterial();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtReplaceMaterial GetModelByCache(int id)
		{
			
			string CacheKey = "smtReplaceMaterialModel-" + id;
			...
			return (Model.smtReplaceMaterial)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtReplaceMaterial> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtReplaceMaterial> DataTableToList(DataTable dt)
		{
			List<Model.smtReplaceMaterial> modelList = new List<Model.smtReplaceMaterial>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtReplaceMaterial model;
