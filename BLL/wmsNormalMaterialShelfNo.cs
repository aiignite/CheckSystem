...
namespace BLL
{
	...
	{
		private readonly DAL.wmsNormalMaterialShelfNo dal=new DAL.wmsNormalMaterialShelfNo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.wmsNormalMaterialShelfNo GetModelByCache(int id)
		{
			
			string CacheKey = "wmsNormalMaterialShelfNoModel-" + id;
			...
			return (Model.wmsNormalMaterialShelfNo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.wmsNormalMaterialShelfNo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.wmsNormalMaterialShelfNo> DataTableToList(DataTable dt)
		{
			List<Model.wmsNormalMaterialShelfNo> modelList = new List<Model.wmsNormalMaterialShelfNo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.wmsNormalMaterialShelfNo model;
