...
namespace BLL
{
	...
	{
		private readonly DAL.qsProductInfo dal=new DAL.qsProductInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.qsProductInfo GetModelByCache(int id)
		{
			
			string CacheKey = "qsProductInfoModel-" + id;
			...
			return (Model.qsProductInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.qsProductInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.qsProductInfo> DataTableToList(DataTable dt)
		{
			List<Model.qsProductInfo> modelList = new List<Model.qsProductInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.qsProductInfo model;
