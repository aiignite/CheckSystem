...
namespace BLL
{
	...
	{
		private readonly DAL.smtRepairRecord dal=new DAL.smtRepairRecord();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtRepairRecord GetModelByCache(int id)
		{
			
			string CacheKey = "smtRepairRecordModel-" + id;
			...
			return (Model.smtRepairRecord)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtRepairRecord> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtRepairRecord> DataTableToList(DataTable dt)
		{
			List<Model.smtRepairRecord> modelList = new List<Model.smtRepairRecord>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtRepairRecord model;
