...
namespace BLL
{
	...
	{
		private readonly DAL.salaryBaseInfo dal=new DAL.salaryBaseInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.salaryBaseInfo GetModelByCache(int id)
		{
			
			string CacheKey = "salaryBaseInfoModel-" + id;
			...
			return (Model.salaryBaseInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.salaryBaseInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.salaryBaseInfo> DataTableToList(DataTable dt)
		{
			List<Model.salaryBaseInfo> modelList = new List<Model.salaryBaseInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.salaryBaseInfo model;
