...
namespace BLL
{
	...
	{
		private readonly DAL.pdModifyRecord dal=new DAL.pdModifyRecord();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.pdModifyRecord GetModelByCache(int id)
		{
			
			string CacheKey = "pdModifyRecordModel-" + id;
			...
			return (Model.pdModifyRecord)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdModifyRecord> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdModifyRecord> DataTableToList(DataTable dt)
		{
			List<Model.pdModifyRecord> modelList = new List<Model.pdModifyRecord>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.pdModifyRecord model;
