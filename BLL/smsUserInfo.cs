...
namespace BLL
{
	...
	{
		private readonly DAL.smsUserInfo dal=new DAL.smsUserInfo();
		public smsUserInfo()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smsUserInfo GetModelByCache(int id)
		{
			
			string CacheKey = "smsUserInfoModel-" + id;
			...
			return (Model.smsUserInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smsUserInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smsUserInfo> DataTableToList(DataTable dt)
		{
			List<Model.smsUserInfo> modelList = new List<Model.smsUserInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smsUserInfo model;
