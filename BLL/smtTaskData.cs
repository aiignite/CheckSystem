...
namespace BLL
{
	...
	{
		private readonly DAL.smtTaskData dal=new DAL.smtTaskData();
		public smtTaskData()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtTaskData GetModelByCache(int id)
		{
			
			string CacheKey = "smtTaskDataModel-" + id;
			...
			return (Model.smtTaskData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtTaskData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtTaskData> DataTableToList(DataTable dt)
		{
			List<Model.smtTaskData> modelList = new List<Model.smtTaskData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtTaskData model;
