...
namespace BLL
{
	...
	{
		private readonly DAL.examQuestions dal=new DAL.examQuestions();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.examQuestions GetModelByCache(int id)
		{
			
			string CacheKey = "examQuestionsModel-" + id;
			...
			return (Model.examQuestions)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.examQuestions> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.examQuestions> DataTableToList(DataTable dt)
		{
			List<Model.examQuestions> modelList = new List<Model.examQuestions>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.examQuestions model;
