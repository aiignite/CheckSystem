...
namespace BLL
{
	...
	{
		private readonly DAL.LogisticsAndon dal=new DAL.LogisticsAndon();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.LogisticsAndon GetModelByCache(int ID)
		{
			
			string CacheKey = "LogisticsAndonModel-" + ID;
			...
			return (Model.LogisticsAndon)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.LogisticsAndon> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.LogisticsAndon> DataTableToList(DataTable dt)
		{
			List<Model.LogisticsAndon> modelList = new List<Model.LogisticsAndon>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.LogisticsAndon model;
