...
namespace BLL
{
	...
	{
		private readonly DAL.pdPFDDetail dal=new DAL.pdPFDDetail();
		public pdPFDDetail()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.pdPFDDetail GetModelByCache(int id)
		{
			
			string CacheKey = "pdPFDDetailModel-" + id;
			...
			return (Model.pdPFDDetail)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdPFDDetail> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdPFDDetail> DataTableToList(DataTable dt)
		{
			List<Model.pdPFDDetail> modelList = new List<Model.pdPFDDetail>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.pdPFDDetail model;
