...
namespace BLL
{
	...
	{
		private readonly DAL.processParaData dal=new DAL.processParaData();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.processParaData GetModelByCache(int id)
		{
			
			string CacheKey = "processParaDataModel-" + id;
			...
			return (Model.processParaData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.processParaData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.processParaData> DataTableToList(DataTable dt)
		{
			List<Model.processParaData> modelList = new List<Model.processParaData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.processParaData model;
