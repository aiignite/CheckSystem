...
namespace BLL
{
	...
	{
		private readonly DAL.devicePartInfo dal=new DAL.devicePartInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.devicePartInfo GetModelByCache(int id)
		{
			
			string CacheKey = "devicePartInfoModel-" + id;
			...
			return (Model.devicePartInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.devicePartInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.devicePartInfo> DataTableToList(DataTable dt)
		{
			List<Model.devicePartInfo> modelList = new List<Model.devicePartInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.devicePartInfo model;
