...
namespace BLL
{
	...
	{
		private readonly DAL.processParaInfo dal=new DAL.processParaInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.processParaInfo GetModelByCache(int id)
		{
			
			string CacheKey = "processParaInfoModel-" + id;
			...
			return (Model.processParaInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.processParaInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.processParaInfo> DataTableToList(DataTable dt)
		{
			List<Model.processParaInfo> modelList = new List<Model.processParaInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.processParaInfo model;
