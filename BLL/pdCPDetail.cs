...
namespace BLL
{
	...
	{
		private readonly DAL.pdCPDetail dal=new DAL.pdCPDetail();
		public pdCPDetail()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.pdCPDetail GetModelByCache(int id)
		{
			
			string CacheKey = "pdCPDetailModel-" + id;
			...
			return (Model.pdCPDetail)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdCPDetail> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdCPDetail> DataTableToList(DataTable dt)
		{
			List<Model.pdCPDetail> modelList = new List<Model.pdCPDetail>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.pdCPDetail model;
