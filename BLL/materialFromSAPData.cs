...
namespace BLL
{
	...
	{
		private readonly DAL.materialFromSAPData dal=new DAL.materialFromSAPData();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.materialFromSAPData GetModelByCache(int id)
		{
			
			string CacheKey = "materialFromSAPDataModel-" + id;
			...
			return (Model.materialFromSAPData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.materialFromSAPData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.materialFromSAPData> DataTableToList(DataTable dt)
		{
			List<Model.materialFromSAPData> modelList = new List<Model.materialFromSAPData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.materialFromSAPData model;
