...
namespace BLL
{
	...
	{
		private readonly DAL.wmsProductShelfNo dal=new DAL.wmsProductShelfNo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.wmsProductShelfNo GetModelByCache(int id)
		{
			
			string CacheKey = "wmsProductShelfNoModel-" + id;
			...
			return (Model.wmsProductShelfNo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.wmsProductShelfNo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.wmsProductShelfNo> DataTableToList(DataTable dt)
		{
			List<Model.wmsProductShelfNo> modelList = new List<Model.wmsProductShelfNo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.wmsProductShelfNo model;
