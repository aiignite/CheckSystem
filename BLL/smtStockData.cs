...
namespace BLL
{
	...
	{
		private readonly DAL.smtStockData dal=new DAL.smtStockData();
		public smtStockData()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtStockData GetModelByCache(int id)
		{
			
			string CacheKey = "smtStockDataModel-" + id;
			...
			return (Model.smtStockData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtStockData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtStockData> DataTableToList(DataTable dt)
		{
			List<Model.smtStockData> modelList = new List<Model.smtStockData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtStockData model;
