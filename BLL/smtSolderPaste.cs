...
namespace BLL
{
	...
	{
		private readonly DAL.smtSolderPaste dal=new DAL.smtSolderPaste();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtSolderPaste GetModelByCache(int id)
		{
			
			string CacheKey = "smtSolderPasteModel-" + id;
			...
			return (Model.smtSolderPaste)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtSolderPaste> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtSolderPaste> DataTableToList(DataTable dt)
		{
			List<Model.smtSolderPaste> modelList = new List<Model.smtSolderPaste>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtSolderPaste model;
