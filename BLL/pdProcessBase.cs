...
namespace BLL
{
	...
	{
		private readonly DAL.pdProcessBase dal=new DAL.pdProcessBase();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.pdProcessBase GetModelByCache(int id)
		{
			
			string CacheKey = "pdProcessBaseModel-" + id;
			...
			return (Model.pdProcessBase)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdProcessBase> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.pdProcessBase> DataTableToList(DataTable dt)
		{
			List<Model.pdProcessBase> modelList = new List<Model.pdProcessBase>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.pdProcessBase model;
