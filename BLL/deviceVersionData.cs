...
namespace BLL
{
	...
	{
		private readonly DAL.deviceVersionData dal=new DAL.deviceVersionData();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.deviceVersionData GetModelByCache(int id)
		{
			
			string CacheKey = "deviceVersionDataModel-" + id;
			...
			return (Model.deviceVersionData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.deviceVersionData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.deviceVersionData> DataTableToList(DataTable dt)
		{
			List<Model.deviceVersionData> modelList = new List<Model.deviceVersionData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.deviceVersionData model;
