...
namespace BLL
{
	...
	{
		private readonly DAL.smtManufactureData dal=new DAL.smtManufactureData();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtManufactureData GetModelByCache(int id)
		{
			
			string CacheKey = "smtManufactureDataModel-" + id;
			...
			return (Model.smtManufactureData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtManufactureData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtManufactureData> DataTableToList(DataTable dt)
		{
			List<Model.smtManufactureData> modelList = new List<Model.smtManufactureData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtManufactureData model;
