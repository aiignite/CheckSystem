...
namespace BLL
{
	...
	{
		private readonly DAL.smtTaskComputer dal=new DAL.smtTaskComputer();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtTaskComputer GetModelByCache(int id)
		{
			
			string CacheKey = "smtTaskComputerModel-" + id;
			...
			return (Model.smtTaskComputer)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtTaskComputer> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtTaskComputer> DataTableToList(DataTable dt)
		{
			List<Model.smtTaskComputer> modelList = new List<Model.smtTaskComputer>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtTaskComputer model;
