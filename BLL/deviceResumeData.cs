...
namespace BLL
{
	...
	{
		private readonly DAL.deviceResumeData dal=new DAL.deviceResumeData();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.deviceResumeData GetModelByCache(int id)
		{
			
			string CacheKey = "deviceResumeDataModel-" + id;
			...
			return (Model.deviceResumeData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.deviceResumeData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.deviceResumeData> DataTableToList(DataTable dt)
		{
			List<Model.deviceResumeData> modelList = new List<Model.deviceResumeData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.deviceResumeData model;
