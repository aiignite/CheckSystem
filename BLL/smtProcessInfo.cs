...
namespace BLL
{
	...
	{
		private readonly DAL.smtProcessInfo dal=new DAL.smtProcessInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtProcessInfo GetModelByCache(int id)
		{
			
			string CacheKey = "smtProcessInfoModel-" + id;
			...
			return (Model.smtProcessInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtProcessInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtProcessInfo> DataTableToList(DataTable dt)
		{
			List<Model.smtProcessInfo> modelList = new List<Model.smtProcessInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtProcessInfo model;
