...
namespace BLL
{
	...
	{
		private readonly DAL.smtProcessRecord dal=new DAL.smtProcessRecord();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtProcessRecord GetModelByCache(int id)
		{
			
			string CacheKey = "smtProcessRecordModel-" + id;
			...
			return (Model.smtProcessRecord)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtProcessRecord> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtProcessRecord> DataTableToList(DataTable dt)
		{
			List<Model.smtProcessRecord> modelList = new List<Model.smtProcessRecord>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtProcessRecord model;
