...
namespace BLL
{
	...
	{
		private readonly DAL.productParaInfo dal=new DAL.productParaInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.productParaInfo GetModelByCache(int id)
		{
			
			string CacheKey = "productParaInfoModel-" + id;
			...
			return (Model.productParaInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.productParaInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.productParaInfo> DataTableToList(DataTable dt)
		{
			List<Model.productParaInfo> modelList = new List<Model.productParaInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.productParaInfo model;
