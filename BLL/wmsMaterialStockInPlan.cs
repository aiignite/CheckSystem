...
namespace BLL
{
	...
	{
		private readonly DAL.wmsMaterialStockInPlan dal=new DAL.wmsMaterialStockInPlan();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.wmsMaterialStockInPlan GetModelByCache(int id)
		{
			
			string CacheKey = "wmsMaterialStockInPlanModel-" + id;
			...
			return (Model.wmsMaterialStockInPlan)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.wmsMaterialStockInPlan> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.wmsMaterialStockInPlan> DataTableToList(DataTable dt)
		{
			List<Model.wmsMaterialStockInPlan> modelList = new List<Model.wmsMaterialStockInPlan>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.wmsMaterialStockInPlan model;
