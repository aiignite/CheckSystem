...
namespace BLL
{
	...
	{
		private readonly DAL.smtStockDataLog dal=new DAL.smtStockDataLog();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.smtStockDataLog GetModelByCache(int id)
		{
			
			string CacheKey = "smtStockDataLogModel-" + id;
			...
			return (Model.smtStockDataLog)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtStockDataLog> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.smtStockDataLog> DataTableToList(DataTable dt)
		{
			List<Model.smtStockDataLog> modelList = new List<Model.smtStockDataLog>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.smtStockDataLog model;
