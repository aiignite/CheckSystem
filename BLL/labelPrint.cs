...
namespace BLL
{
	...
	{
		private readonly DAL.labelPrint dal=new DAL.labelPrint();
		public labelPrint()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.labelPrint GetModelByCache(int id)
		{
			
			string CacheKey = "labelPrintModel-" + id;
			...
			return (Model.labelPrint)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.labelPrint> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.labelPrint> DataTableToList(DataTable dt)
		{
			List<Model.labelPrint> modelList = new List<Model.labelPrint>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.labelPrint model;
