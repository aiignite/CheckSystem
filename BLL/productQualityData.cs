...
namespace BLL
{
	...
	{
		private readonly DAL.productQualityData dal=new DAL.productQualityData();
		...
		{
			return dal.DeleteList(DBUtility.PageValidate.SafeLongFilter(DIDlist,0) );
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.productQualityData GetModelByCache(int DID)
		{
			
			string CacheKey = "productQualityDataModel-" + DID;
			...
			return (Model.productQualityData)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.productQualityData> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.productQualityData> DataTableToList(DataTable dt)
		{
			List<Model.productQualityData> modelList = new List<Model.productQualityData>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.productQualityData model;
