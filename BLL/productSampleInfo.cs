...
namespace BLL
{
	...
	{
		private readonly DAL.productSampleInfo dal=new DAL.productSampleInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.productSampleInfo GetModelByCache(int id)
		{
			
			string CacheKey = "productSampleInfoModel-" + id;
			...
			return (Model.productSampleInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.productSampleInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.productSampleInfo> DataTableToList(DataTable dt)
		{
			List<Model.productSampleInfo> modelList = new List<Model.productSampleInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.productSampleInfo model;
