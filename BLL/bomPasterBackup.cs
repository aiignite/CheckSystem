...
namespace BLL
{
	...
	{
		private readonly DAL.bomPasterBackup dal=new DAL.bomPasterBackup();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.bomPasterBackup GetModelByCache(int id)
		{
			
			string CacheKey = "bomPasterBackupModel-" + id;
			...
			return (Model.bomPasterBackup)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.bomPasterBackup> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.bomPasterBackup> DataTableToList(DataTable dt)
		{
			List<Model.bomPasterBackup> modelList = new List<Model.bomPasterBackup>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.bomPasterBackup model;
