...
namespace BLL
{
	...
	{
		private readonly DAL.mesDeviceCheckInfo dal=new DAL.mesDeviceCheckInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.mesDeviceCheckInfo GetModelByCache(int id)
		{
			
			string CacheKey = "mesDeviceCheckInfoModel-" + id;
			...
			return (Model.mesDeviceCheckInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.mesDeviceCheckInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.mesDeviceCheckInfo> DataTableToList(DataTable dt)
		{
			List<Model.mesDeviceCheckInfo> modelList = new List<Model.mesDeviceCheckInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.mesDeviceCheckInfo model;
