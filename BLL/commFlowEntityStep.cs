...
namespace BLL
{
	...
	{
		private readonly DAL.commFlowEntityStep dal=new DAL.commFlowEntityStep();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.commFlowEntityStep GetModelByCache(int id)
		{
			
			string CacheKey = "commFlowEntityStepModel-" + id;
			...
			return (Model.commFlowEntityStep)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.commFlowEntityStep> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.commFlowEntityStep> DataTableToList(DataTable dt)
		{
			List<Model.commFlowEntityStep> modelList = new List<Model.commFlowEntityStep>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.commFlowEntityStep model;
