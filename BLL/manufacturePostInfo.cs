...
namespace BLL
{
	...
	{
		private readonly DAL.manufacturePostInfo dal=new DAL.manufacturePostInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.manufacturePostInfo GetModelByCache(int id)
		{
			
			string CacheKey = "manufacturePostInfoModel-" + id;
			...
			return (Model.manufacturePostInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.manufacturePostInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.manufacturePostInfo> DataTableToList(DataTable dt)
		{
			List<Model.manufacturePostInfo> modelList = new List<Model.manufacturePostInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.manufacturePostInfo model;
