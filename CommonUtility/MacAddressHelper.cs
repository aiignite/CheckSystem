...
namespace CommonUtility
{
    public static class MacAddressHelper
    {
        ...
        public static string GetMacByIpConfig()
        {
            ...
            foreach (
                var line in
                    runCmd.Split(Environment.NewLine.ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
                        .Select(l => l.Trim())
        ...
        public static List<string> GetMacByWmi()
        {
            try
            {
                ...
                return (from ManagementObject mo in moc where (bool)mo["IPEnabled"] select mo["MacAddress"].ToString()).ToList();
            }
            catch (Exception)
            {
                return null;
