...
namespace CommonUtility.WlanOperator
{
    /// <summary>
    /// Wlan小助手
    /// </summary>
    ...
    {
        public static async void ConnectTargetWifi(string ssid, string password)
        {
            await Task.Run(() =>
            {
                ...
                foreach (var wlanInterface in wlanClient.Interfaces)
                {
                    ...
                    {
                        var getSsid = GetStringForSsid(network.dot11Ssid);

                        if (getSsid != ssid)
                            continue;

                        ConnetWifi(wlanInterface, network, password);
                        return;
                    }
                }
            });
        ...
        {
            var profile = GetStringForSsid(network.dot11Ssid);
            if (password != null)
            {
                var hex = StringToHex(profile);
                var authentication = GetAuthentication(network.dot11DefaultAuthAlgorithm);
                var authEncryption = GetEncryption(network.dot11DefaultCipherAlgorithm);
                var keytype = GetKeyType(authEncryption);
                var profileXml = string.Format("<?xml version=\"1.0\"?><WLANProfile xmlns=\"http://www.microsoft.com/networking/WLAN/profile/v1\"><name>{0}</name><SSIDConfig><SSID><hex>{1}</hex><name>{0}</name></SSID></SSIDConfig><connectionType>ESS</connectionType><connectionMode>auto</connectionMode><MSM><security><authEncryption><authentication>{2}</authentication><encryption>{3}</encryption><useOneX>false</useOneX></authEncryption><sharedKey><keyType>{4}</keyType><protected>false</protected><keyMaterial>{5}</keyMaterial></sharedKey></security></MSM></WLANProfile>", profile, hex, authentication, authEncryption, keytype, password);
            ...
            else
            {
                ...
                {
                    var hex = StringToHex(profile);
                    var profileXml = string.Format("<?xml version=\"1.0\"?><WLANProfile xmlns=\"http://www.microsoft.com/networking/WLAN/profile/v1\"><name>{0}</name><SSIDConfig><SSID><hex>{1}</hex><name>{0}</name></SSID></SSIDConfig><connectionType>ESS</connectionType><connectionMode>auto</connectionMode><MSM><security><authEncryption><authentication>open</authentication><encryption>none</encryption><useOneX>false</useOneX></authEncryption></security></MSM></WLANProfile>", profile, hex);
                    wlanIface.SetProfile(Wlan.WlanProfileFlags.AllUser, profileXml, true);
                    wlanIface.Connect(Wlan.WlanConnectionMode.Profile, Wlan.Dot11BssType.Any, profile);
                }
            }
        ...
        {
            var p = wlanIface.GetProfiles();
            return p.Any(item => item.profileName == profile);
        }
        ...
        public static string StringToHex(string str)
        {
            ...
            var byStr = Encoding.Default.GetBytes(str); //默认是System.Text.Encoding.Default.GetBytes(str)
