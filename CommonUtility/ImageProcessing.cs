...
namespace CommonUtility
{
    ...
    public static class ImageProcessing
    {
        ...
        public static List<CircleReport> GetCircle(
            VisionImage imageSource, Range radiusRange, Range thresholdRange)
        {
            using (var circleImg = new VisionImage(ImageType.U8, 7))
            {
                ...
                var circleResults = Algorithms.FindCircles(img, circleImg, radiusRange);
            ...
            try
            {
                using (var roi = new Roi { shape })
                {
                    ...
                    using (var imageMask = new VisionImage(ImageType.U8, 7))
                    {
                        ...
                        Algorithms.RoiToMask(imageMask, roi, new PixelValue(255), imageSource);
           ...
           byte[] source, int width, int heigh, bool iscolor)
        {
            ...
            {
                Console.WriteLine(@"ConvertBytesToVisionImg error: " + ex.Message);
                var returnErrorImage = new VisionImage(iscolor ? ImageType.Rgb32 : ImageType.U8);
                returnErrorImage.SetSize(width, heigh);
                return returnErrorImage;
            }

            //st.Stop();
            //Console.WriteLine("ConvertBytesToVisionImg cost = {0}ms", st.ElapsedMilliseconds);
