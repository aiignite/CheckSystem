...
namespace UserControls.CvDisplaySrc
{
    class CvDisplayGraphicsLineSegment : CvDisplayGraphicsLine
    {
        ...
        public double Length
        {
            get
            {
                return Calculate.Distance(Start, End);
            }
        }
        ...
        public CvDisplayGraphicsLineSegment() :
            this(0, 0, 0, 0) { }
        ...
        {
            var startDot = new CvDisplayGraphicsDot(Start) { ParentMat = ParentMat };
            var endDot = new CvDisplayGraphicsDot(End) { ParentMat = ParentMat };

            var rectPos = TranslateToScreenPos(Location);
            var length = Calculate.Distance(TranslateToScreenPos(Start), TranslateToScreenPos(End));
            var rotateRect = new RotatedRect(new Point2f((float)rectPos.X, (float)rectPos.Y),
            ...
            else if (Calculate.RotateRectContains(rotateRect, new Point2d(pos.X, pos.Y)))
        ...
        public override void OnMouseMove(MouseEventArgs e)
        {
            if (IsLeftMouseDown)
            {
                ...
                {
                    var screenStart = TranslateToScreenPos(Start);
                    screenStart.X += xmove;
                    screenStart.Y += ymove;
                ...
                {
                    var screenEnd = TranslateToScreenPos(End);
                    screenEnd.X += xmove;
                    screenEnd.Y += ymove;
        ...
        public override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                var brush = new SolidBrush(GetDrawColor());
                float radius = 3;
