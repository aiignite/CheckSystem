...
namespace HZH_Controls.Controls.DateTime
{
    ...
    public partial class UCCalendarNotes_Week : UserControl
    {
        ...
        public delegate void AddNoteEvent(System.DateTime beginTime);
        /// <summary>
        /// 点击节点时间
        /// </summary>
        ...
        private void ResetCurrentDataSource()
        {
            ...
            if (_dataSource != null)
            {
                ...
                {
                    System.DateTime dt2 = System.DateTime.Parse(item.EndTime.Value.ToString("yyyy-MM-dd") + " " + item.EndTime.Value.Hour + ":00:00");
                    int beginHour = 0;
                    if (item.BeginTime.Day == CurrentTime.Day)
                        beginHour = item.BeginTime.Hour;
                    for (int i = beginHour; i < 24; i++)
                    {
                        System.DateTime dt1 = System.DateTime.Parse(item.BeginTime.ToString("yyyy-MM-dd") + " " + i + ":00:00");
                        if (dt1 <= dt2)
                        {
                ...
                if (m_lstHourSource.ContainsKey(i))
                {
                    ...
                    foreach (var item in lst)
                    {
                        if ((item.BeginTime.Hour == i && item.BeginTime.Day == CurrentTime.Day) || i == 0)//如果开始时间在当前区间测画矩形
                        {
                            ...
                            int h1 = (int)((item.EndTime.Value - (item.BeginTime.Day == CurrentTime.Day ? item.BeginTime : System.DateTime.Parse(CurrentTime.ToString("yyyy-MM-dd 00:00:00")))).TotalMinutes);
                            int h2 = (int)((System.DateTime.Parse(CurrentTime.ToString("yyyy-MM-dd 23:59:59")) - (item.BeginTime.Day == CurrentTime.Day ? item.BeginTime : System.DateTime.Parse(CurrentTime.ToString("yyyy-MM-dd 00:00:00")))).TotalMinutes);
                            int h = h1;
                            if (h1 > h2)
                                h = h2;
        ...
        private void SetTimeToControl()
        {
            ...
            System.DateTime _dt = m_dt.AddDays(-week);
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                foreach (var item in m_lstAddRect)
                {
                    if (item.Value.Contains(e.Location))
                    {
                        if (AddClick != null)
                        {
                            AddClick(System.DateTime.Parse(CurrentTime.ToString("yyyy-MM-dd") + " " + item.Key + ":00:00"));
                            panMain.Invalidate();
                        }
                        return;
                    }
        ...
        private void lblWeek_Click(object sender, EventArgs e)
        {
            ...
            CurrentTime = (System.DateTime)c.Tag;
        }
        ...
        private void lblAdd_Click(object sender, EventArgs e)
        {
            if (AddClick != null)
            {
                AddClick(System.DateTime.Parse(CurrentTime.ToString("yyyy-MM-dd")));
                panMain.Invalidate();
            }
        }
    }
