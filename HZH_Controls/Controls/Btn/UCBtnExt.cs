...
namespace HZH_Controls.Controls.Btn
{
    ...
    {
        #region 字段属性
        private bool _enabledMouseEffect;

        [Description("是否启用鼠标效果"), Category("自定义")]
        public bool EnabledMouseEffect
        {
            get { return _enabledMouseEffect; }
            set { _enabledMouseEffect = value; }
        }
        ...
        /// The m tips color
        /// </summary>
        private Color _mTipsColor = Color.FromArgb(232, 30, 99);
        ...
        {
            get { return _mTipsColor; }
            set { _mTipsColor = value; }
        }
        ...
        private Color _mCacheColor = Color.Empty;

        private void lbl_MouseLeave(object sender, EventArgs e)
        {
            if (!_enabledMouseEffect)
                return;
            if (MouseEffecting != null && MouseEffected != null)
                MouseEffected(this, e);
            else
            {
                if (_mCacheColor == Color.Empty)
                    return;
                FillColor = _mCacheColor;
                _mCacheColor = Color.Empty;
            }
        }
        ...
        private void lblTips_Paint(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.FillEllipse(new SolidBrush(_mTipsColor),
                new Rectangle(0, 0, lblTips.Width - 1, lblTips.Height - 1));
            var sizeEnd = e.Graphics.MeasureString(TipsText, lblTips.Font);

            e.Graphics.DrawString(TipsText, lblTips.Font, new SolidBrush(lblTips.ForeColor),
                new PointF((lblTips.Width - sizeEnd.Width) / 2, (lblTips.Height - sizeEnd.Height) / 2 + 1));
        }
