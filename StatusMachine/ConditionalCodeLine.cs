                    ...
                    foreach (var t in lstCodeLineOr)
                    {
                        ...
                        for (var i = 0; i < lstCodeLineAnd.Count; i++)
                        {
                                ...
                                right.Equals("0"))) // 1. 和参数作比较
                            {
                                ...
                                {
                                    var ppp = DeviceConfig.Paras is null ? (new DeviceConfigPara[0]).ToList().Find(f => f.ProcessNo == processNo && f.Name == paraName) : DeviceConfig.Paras.ToList().Find(f => f.ProcessNo == processNo && f.Name == paraName);
                                    if (ppp is null)
                                    {
                                        List<CheckData> c2;
                                        bool isP2;
            ...
            else if (expectedPara.DataType.ToLower() is "roi") // 单帧
            {
                ...
                if (comparedValue.GetType() == typeof(Dictionary<string, Tuple<long, string>[]>))
                {
                    ...
                    if (dic != null && dic.ContainsKey(key) && dic[key] != null && dic[key].Any() && rois.Any())
                    {
                        ...
                        for (var tryingIndex = 0; tryingIndex < retryCount; tryingIndex++)
                        {
                            ...
                            if (findRoiBuffer is null)
                            {
                                ...
                                else
                                {
                                    ...
                                    try
                                    {
                                        for (var index = 0; index < roiBuffer.Rects.Count; index++)
                                        {
                                            var tshape = roiBuffer.Rects[index];
                                            ...
                                            try
                                            {
                                                ...
                                                roiBuffer.Rects[index].Gray = Math.Round(grayPerMat, 2, MidpointRounding.AwayFromZero);
                                            }
                                            catch (Exception ex)
                                            {
                        ...
                        {
                            var scaleX = (double)800 / matShow.Width;
                            var scaleY = (double)600 / matShow.Height;
                            var scale = scaleX >= scaleY ? scaleX : scaleY;
                            matShow = matShow.Resize(new Size(), scale, scale);
                        }
