...
namespace CheckSystem.SmtForms.DataUploader
{
    public class MyQueueEvent
    {
        ...
        private void FileSystemWatcher_Created(object sender, FileSystemEventArgs e)
        {
            ...
            if (fileSystemWatcher != null)
            {
                if (fileSystemWatcher.Path == Config.Folder)
                {
                    ...
                    else if (Config.Type == "TriAOI" && e.FullPath.ToLower().EndsWith(".txt") && e.Name.ToLower().Contains("_Board_".ToLower()))
                    {
        ...
        /// <returns></returns>
        private Func<PostDetail[]> TriAoiConvert(string boardFilePath)
        {
            return () =>
            {
                try
                {
                    ...
                    var sp = boardFilePath.Split(new[] { "_Board_" }, StringSplitOptions.RemoveEmptyEntries);
                    var fovCompFilePath = string.Format(@"{0}_FovComp_{1}", sp[0], sp[1]);
                    ...
                    if (File.Exists(fovCompFilePath))
                    {
