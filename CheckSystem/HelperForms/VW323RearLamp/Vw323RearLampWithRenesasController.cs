...
{
    public partial class Vw323RearLampWithRenesasController : UIForm
    {
        ...
        public Vw323RearLampWithRenesasController()
        {
            InitializeComponent();

            LinController = new SyRenesasMcuControllerMaster("LIN控制器");
            ...
            Load += Vw323RearLampWithRenesasController_Load;
            Closed += Vw323RearLampWithRenesasController_Closed;
        }

        private void Vw323RearLampWithRenesasController_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var withSyRenesasMcuControllerMaster = LinController as SyRenesasMcuControllerMaster;
                if (withSyRenesasMcuControllerMaster != null)
                {
                    withSyRenesasMcuControllerMaster.InitRemoteIpAddress("192.168.1.28:8088");
                }
                ...
                IoThread = new Thread(IoWork) { IsBackground = true };
                IoThread.Start();
        ...
        private void Vw323RearLampWithRenesasController_Closed(object sender, System.EventArgs e)
        {
            if (LinMsgThread != null)
            {
                LinMsgThread.Abort();
        ...
        private void MainWork()
        {
            ...
            while (LinMsgThread.IsAlive)
            {
                ...
                if (st.ElapsedMilliseconds > 20)
                {
                    ...
                    if (LinController is SyRenesasMcuControllerMaster)
                    {
                        var syRenesasMcuControllerMaster = LinController as SyRenesasMcuControllerMaster;
                        if (syRenesasMcuControllerMaster.GatewayLin1 == null)
                            continue;
                        linBus = syRenesasMcuControllerMaster.GatewayLin1;
                    }
                    ...
                    if (_isTurnSwitchOnOff)
                    {
                        ...
                        {
                            _isTurnOnOffCount = 0;
                            //Console.WriteLine(@"switch turn at {0}/ms", st.ElapsedMilliseconds);
                        }
                    }
                    ...
                    //var crc = Function_CRC(_intelMatrix.MatrixData, _bzCount);
        ...
        private readonly byte[] _crc8LookupTable =
        {
            ...
            249, 214, 167, 136, 69, 106, 27, 52, 115, 92, 45, 2, 207, 224, 145, 190, 36, 11, 122, 85, 152, 183, 198,
            233, 221, 242, 131, 172, 97, 78, 63, 16, 138, 165, 212, 251, 54, 25, 104, 71, 230, 201, 184, 151, 90, 117,
            4, 43, 177, 158, 239, 192, 13, 34, 83, 124, 72, 103, 22, 57, 244, 219, 170, 133, 31, 48, 65, 110, 163, 140,
            253, 210, 149, 186, 203, 228, 41, 6, 119, 88, 194, 237, 156, 179, 126, 81, 32, 15, 59, 20, 101, 74, 135,
            168, 217, 246, 108, 67, 50, 29, 208, 255, 142, 161, 227, 204, 189, 146, 95, 112, 1, 46, 180, 155, 234, 197,
            8, 39, 86, 121, 77, 98, 19, 60, 241, 222, 175, 128, 26, 53, 68, 107, 166, 137, 248, 215, 144, 191, 206, 225,
            44, 3, 114, 93, 199, 232, 153, 182, 123, 84, 37, 10, 62, 17, 96, 79, 130, 173, 220, 243, 105, 70, 55, 24,
            213, 250, 139, 164, 5, 42, 91, 116, 185, 150, 231, 200, 82, 125, 12, 35, 238, 193, 176, 159, 171, 132, 245,
            218, 23, 56, 73, 102, 252, 211, 162, 141, 64, 111, 30, 49, 118, 89, 40, 7, 202, 229, 148, 187, 33, 14, 127,
            80, 157, 178, 195, 236, 216, 247, 134, 169, 100, 75, 58, 21, 143, 160, 209, 254, 51, 28, 109, 66
        };

        /// <summary>
        /// DataID
        /// </summary>
        private readonly byte[] _bcm2SbbRe01Table =
        {
            0x13, 0x33, 0x7e, 0x41, 0x09, 0xe7, 0x85,
            0x46, 0xad, 0x03, 0x12, 0xd3, 0xb7, 0xb8, 0xd7, 0x58
        };

        private byte Function_CRC(IReadOnlyList<byte> data, byte cz)
        {
            byte checkSum = 0xFF;
            for (var index = 1; index < 8; index++)
            {
                checkSum = (byte)((_crc8LookupTable[checkSum ^ data[index]]) & 0xFF);
            }
            checkSum = (byte)((_crc8LookupTable[checkSum ^ _bcm2SbbRe01Table[cz]]) & 0xFF);
            checkSum ^= 0xFF;
            return checkSum;
        }
        ...
        private readonly SyRenesasMcuControllerMaster _dpController = new SyRenesasMcuControllerMaster("低配控制器主站IP29");
        private readonly SyRenesasMcuControllerSlaveWith8RLs _dpRlController = new SyRenesasMcuControllerSlaveWith8RLs("低配RL控制器");
        private bool _isDpTurnOnOff;
        private int _isDpTurnOnOffCount;

        private Thread IoThread { get; set; }

        private void IoWork()
        {
            _dpController.InitRemoteIpAddress("192.168.1.29:8088");
            _dpRlController.ConnectMaster("192.168.1.29", "0x201");
            _dpRlController.ResetOutPuts();
            ...
            while (IoThread.IsAlive)
            {
                ...
                if (st.ElapsedMilliseconds >= 20)
                {
                    if (_isDpTurnOnOff)
                    {
                        ...
                        if (swDpTurn.Active)
                        {
                            if (_isDpTurnOnOffCount == 20)
                                _dpRlController.Relay2 = false;
                            else if (_isDpTurnOnOffCount == 40)
                                _dpRlController.Relay2 = true;
                        }

                        if (_isDpTurnOnOffCount > 40)
                            _isDpTurnOnOffCount = 0;
                    }

                    _dpRlController.SetOutputs();
                    st.Restart();
                }
            }
        }

        private void swDpTail_ActiveChanged(object sender, EventArgs e)
        {
            _dpRlController.Relay1 = swDpTail.Active;
        }

        private void swDpFog_ActiveChanged(object sender, EventArgs e)
        {
            _dpRlController.Relay4 = swDpFog.Active;
        }

        private void swDpStop_ActiveChanged(object sender, EventArgs e)
        {
            _dpRlController.Relay3 = swDpStop.Active;
        }

        private void swDpBul_ActiveChanged(object sender, EventArgs e)
        {
            _dpRlController.Relay5 = swDpBul.Active;
        }

        private void swDpTurn_ActiveChanged(object sender, EventArgs e)
        {
            _dpRlController.Relay2 = swDpTurn.Active;
        }

        private void swDpTurnOnOff_ActiveChanged(object sender, EventArgs e)
        {
            if (swDpTurnOnOff.Active)
            {
                swDpTurn.Enabled = false;
                _isDpTurnOnOffCount = 0;
                _isDpTurnOnOff = true;
            }
            else
            {
                swDpTurn.Active = false;
                swDpTurn.Enabled = true;
                _isDpTurnOnOff = false;
                _isDpTurnOnOffCount = 0;
            }
        }
