...
//using P12LLowLevelHeadLamp = CheckSystem.HelperForms.P12LHeadLamp.P12LLowLevelHeadLamp;

namespace CheckSystem.HelperForms.S12LHeadLamp
{
    public partial class S12LHeadLampDeviceMgr : Form
    {
        ...
        private void btnConnect_BtnClick(object sender, EventArgs e)
        {
            try
            {
                if (ucmbProductLevel.comboBox.SelectedIndex == 0) // 高配
                {
                    ...
                    using (var frm = new S12LHighLevelHeadLamp(InitCan1, InitCan2, InitLin6))
                        frm.ShowDialog();
                    Close();
                }
                else // 低配
