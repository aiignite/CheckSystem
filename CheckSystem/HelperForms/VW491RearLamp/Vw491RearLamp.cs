...
{
    public sealed partial class Vw491RearLamp : UIForm
    {
        ...
        private bool _isRenesas = false;
        private bool _isToomoss = false;
        ...
        public Vw491RearLamp(bool isToomoss, bool isRenesas = false)
        {
            InitializeComponent();
            _isRenesas = isRenesas;
            _isToomoss = isToomoss;

            if (!isToomoss)
            {
                if (isRenesas)
                {
                    LinController = new SyRenesasMcuControllerMaster("Renesas");
                }
                else
                {
            ...
            _matrixValDefinitions.Add(SBBR_Helligkeit_Dunkel, new MatrixValDefinition(42, 1, 1));
        ...
        private void Vw491RearLamp_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    if (_isRenesas)
                    {
                        var withRenesas = LinController as SyRenesasMcuControllerMaster;
                        if (withRenesas != null)
                        {
                            var pin = withRenesas;
        ...
        private void Vw491RearLamp_Closed(object sender, EventArgs e)
        {
            if (LinMsgThread != null)
            {
                LinMsgThread.Abort();
        ...
        private void MainWork()
        {
            ...
            while (LinMsgThread.IsAlive)
            {
                ...
                else if (LinController is SyRenesasMcuControllerMaster)
                {
                ...
                {
                    case 0:
                        linBus.SendMasterLin(0x0B, new byte[0]);
                        break;

                    case 1:
                        linBus.SendMasterLin(0x0C, new byte[0]);
                        break;

                    case 2:
                        linBus.SendMasterLin(0x0D, new byte[0]);
                        break;

                    case 3:
                        linBus.SendMasterLin(0x0E, new byte[0]);
                        break;
                }
        ...
        private void LinBus_PushLinMsg(string name, LinBus.LinDataPackage data)
        {
            ...
            if (linBus != null && linBus.Name == name)
            {
        ...
        private void uiSwitchChLhInvoke_ValueChanged(object sender, bool value)
        {
            if (value)
            {
                ...
                if (rdBtnCh.Checked && !rdBtnLh.Checked) // CHO动画，转向灯闪烁两次
                {
                    ...
                    _matrixValDefinitions[BCM1_CH_aktiv].Value = 0x01;
                    _matrixValDefinitions[BCM1_LH_aktiv].Value = 0x00;
                }
                else if (!rdBtnCh.Checked && rdBtnLh.Checked) // LHO动画，转向灯闪烁两次
                {
                    ...
                    _matrixValDefinitions[BCM1_CH_aktiv].Value = 0x00;
                    _matrixValDefinitions[BCM1_LH_aktiv].Value = 0x01;
                }
            }
            else
            {
